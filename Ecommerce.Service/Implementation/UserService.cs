...
namespace Ecommerce.Service.Implementation
{
    ...
    {
        private readonly IGenericRepository<Usuario> _modeloRepositorio;
        private readonly IMapper _mapper;

        public UserService(IGenericRepository<Usuario> modeloRepositorio, IMapper mapper)
        {
        ...
        public async Task<UsuarioDTO> Crear(UsuarioDTO modelo)
        {
            try
            {
                var dbModel = _mapper.Map<Usuario>(modelo);
                var rspModelo = await _modeloRepositorio.Crear(dbModel);
        ...
        public async Task<bool> Eliminar(int id)
        {
            try
            {
                ...
                if (fromDbModel != null)
                {
                    var respuesta = await _modeloRepositorio.Eliminar(fromDbModel);
                    if (!respuesta)
